...
### Added
- **Lixeira** (SAFETY-03): apagar um gasto, receita, item guardado, produto da wishlist ou conta/dívida deixa de ser definitivo — fica marcado (`deleted_at`) e some das listas/totais, mas fica acessível e restaurável em Configurações → Lixeira. O motor financeiro, o backup e todas as listagens passam a filtrar sempre `deleted_at is null`.
- **Histórico de alterações** (SAFETY-02): nova tabela `activity_log` regista criar/editar/apagar/restaurar gasto ou receita, marcar pago/parcial/pendente, converter wishlist, fechar/reabrir mês e exportar/importar backup — consultável em Configurações → Histórico de alterações.
- **Aviso de mês fechado** (FINANCE-15): editar um gasto ou receita datado num mês já fechado mostra um aviso (não bloqueia, apenas avisa que o fechamento guardado fica desatualizado).
- **Gastos rápidos favoritos** (UX-04): atalhos configuráveis (ex.: Café, Pão, Uber) em Registos — um toque lança o gasto na hora, ordenados pelos mais usados.
- **Cartão de crédito** (CREDIT-01): carteiras do tipo Cartão podem guardar dia de fecho, dia de vencimento e limite de crédito (informativo nesta versão — gastos no cartão continuam a contar já no mês; fatura mensal fica para depois).
- **Calculadora de parcela** (INSTALLMENTS-01): ao criar uma conta recorrente com parcelas, dá para indicar o valor total da compra e calcular automaticamente o valor de cada parcela (a base de parcelas X/Y já existia via `end_date`/`installmentLabel`).
- **Relatório mensal** (REPORT-01): exporta o mês (receitas, gastos, saldo, categorias, maior gasto, Gastos Invisíveis, recorrentes, previsão) como HTML imprimível (Imprimir → Guardar como PDF) ou JSON, a partir do card de Fechamento mensal.
